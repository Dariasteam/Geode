...
private:
  ...
  void generate_next_poblation () {
    ...
    auto async_function = std::async([&](unsigned index) {
  ...
  void generate_next_candidates () {
    std::copy (poblation.begin(), poblation.begin() + candidates_size, best_candidates);
  }
...
public:
  ...
  void set_initial_poblation (std::vector<T> i_poblation) {
    ...
    if (diff_size > 0) {    // hay más candidatos de los que se requiere
      std::copy(i_poblation.begin(), i_poblation.end() - diff_size, best_candidates);
